...
class TipCalculator():
    def __init__(self):
        ...
        calculate_btn = Button(window, text= "Clear", bg= "black", fg= "white", command= self.clear)
        calculate_btn.grid(column= 2, row= 7)
        
        window.mainloop()

    # To calculate the tip
    def calculate(self):
        per_tip = float(self.meal_cost.get())
        percentage = self.tip_percent.get() / 100
        tip_amount_entry = per_tip * percentage
        self.tip.set(tip_amount_entry)

        final_bill = per_tip + tip_amount_entry
